...
sed -i "0,/mehran/s//$(whoami)/" $SCRIPT_DIR/flake.nix
sed -i "0,/mehran/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
sed -i "s/m.mehrashahidi@gmail.com//" $SCRIPT_DIR/flake.nix
sed -i "s+~/.nixconf+$SCRIPT_DIR+g" $SCRIPT_DIR/flake.nix
